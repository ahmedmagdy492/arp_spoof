...
static void init() {
    ...
    p_interface.param_desc = "[Required] the interface in which to use to send the packets";

    p_target_ip.param_full_name = "--target_ip";
    p_target_ip.param_short_name = "";
    p_target_ip.param_desc = "[Required] the ip in which the send the packets to";

    p_target_mac.param_full_name = "--target_mac";
    p_target_mac.param_short_name = "";
    p_target_mac.param_desc = "[Required] the mac address in which the send the query about";

    p_spoof_ip.param_full_name = "--spoof_ip";
    p_spoof_ip.param_short_name = "";
    p_spoof_ip.param_desc = "[Required] the ip in which to spoof the victim";
...
static int are_params_valid(char* args[], int argc) {
    ...
    for(int i = 0; i < argc; i++) {
        if(strcmp(args[i], p_help.param_full_name) == 0 || argc < PARAM_REQ_COUNT) {
            printf("ARP Spoof Help Menu\n");
            printf("--------------------------------\n");

            printf("Usage: %s <param-name> [<argument-value>] \n\n", args[0]);
            printf("\nshort_name\tfull_name\t\t\tdesc\n");
        ...
        if(strcmp(args[i], p_help.param_short_name) == 0 || argc < PARAM_REQ_COUNT) {
            printf("ARP Spoof Help Menu\n");
            printf("--------------------------------\n");
...
int parse_args(char* args[], int argc, char* interface, char* target_ip, char* target_mac, char* spoof_ip) {
    ...
    if(are_params_valid(args, argc)) {
        ...
        strncpy(spoof_ip, p_spoof_ip.value, strlen(p_spoof_ip.value)+1);
